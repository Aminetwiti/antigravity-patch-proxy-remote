...
func TestChaos_ReconnectMatrix24_7(t *testing.T) {
 ...
 // Invariant 4 check: sequence numbers must be strictly monotonic
 if len(receivedSequences) > 0 && seq <= receivedSequences[len(receivedSequences)-1] {
 t.Errorf(INVARIANT 4 VIOLATION: Non-monotonic sequence! Got %d after %d, seq, receivedSequences[len(receivedSequences)-1])
 }
 ...
 _, _ = newWsMgr.RegisterWorkspace(ws.ID, ws.Name, ws.Path)
