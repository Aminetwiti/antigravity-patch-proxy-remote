...
// -----------------------------------------------------------------------------
// Autonomous Lifecycle & Self-Healing Extensions
// -----------------------------------------------------------------------------
...
// CreatePullRequest commits any pending changes in the shadow worktree, pushes the branch to origin,
// and either creates a GitHub Pull Request via API (if GITHUB_TOKEN is set) or generates a direct 1-click comparison URL.
func (m *Manager) CreatePullRequest(baseWsID, sessionID, title, body string) (*PullRequestResult, error) {
	if sessionID == "" {
		return nil, fmt.Errorf("sessionID cannot be empty")
	}
	baseWs, err := m.GetWorkspace(baseWsID)
	...
	if token != "" && owner != "" && repo != "" {
		prTitle := title
		if prTitle == "" {
			prTitle = fmt.Sprintf("Agent Changes for Session %s", sessionID)
		}
		prBody := body
		if prBody == "" {
			prBody = fmt.Sprintf("Automated PR generated by Antigravity Remote Cloud Daemon for session `%s`.", sessionID)
		}
		...
		req, reqErr := http.NewRequest("POST", fmt.Sprintf("https://api.github.com/repos/%s/%s/pulls", owner, repo), bytes.NewReader(payload))
		if reqErr == nil {
			req.Header.Set("Authorization", "Bearer "+token)
			req.Header.Set("Accept", "application/vnd.github.v3+json")
			req.Header.Set("Content-Type", "application/json")
			req.Header.Set("User-Agent", "Antigravity-Remote-Daemon")
			...
			if httpErr == nil {
				...
				if resp.StatusCode == http.StatusCreated {
					...
					if err := json.Unmarshal(respBody, &ghResp); err == nil {
						return &PullRequestResult{
							Success:  true,
							PRNumber: ghResp.Number,
							URL:      ghResp.HTMLURL,
							Branch:   shadowBranch,
							Base:     baseBranch,
							Message:  fmt.Sprintf("Pull request #%d created successfully", ghResp.Number),
						}, nil
					}
				}
			}
	...
	return &PullRequestResult{
		...
		Branch:  shadowBranch,
		Base:    baseBranch,
		Message: fmt.Sprintf("Branch %s pushed successfully. 1-click PR comparison URL: %s", shadowBranch, compareURL),
	}, nil
}

func parseGitHubOwnerRepo(remoteURL string) (string, string) {
	clean := strings.TrimSuffix(remoteURL, ".git")
	// Handles git@github.com:owner/repo
...
type PruneResult struct {
	...
	PrunedPaths []string `json:"prunedPaths"`
	Errors      []string `json:"errors,omitempty"`
}

// PruneStaleWorktrees scans worktree directories and removes worktrees older than maxAge.
func (m *Manager) PruneStaleWorktrees(maxAge time.Duration) (*PruneResult, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	// Collect all worktree candidate directories
	var candidateDirs []string
	for _, ws := range m.workspaces {
		candidateDirs = append(candidateDirs,
			filepath.Join(ws.Root, ".antigravity", "worktrees"),
			filepath.Join(filepath.Dir(ws.Root), "worktrees"),
		)
	}
		...
		for _, entry := range entries {
			...
			if info.ModTime().Before(cutoff) {
				if rmErr := os.RemoveAll(wtPath); rmErr != nil {
					result.Errors = append(result.Errors, fmt.Sprintf("failed to remove %s: %v", wtPath, rmErr))
				} else {
					result.PrunedCount++
					result.PrunedPaths = append(result.PrunedPaths, wtPath)
				}
			}
		}
	}

	// Prune git worktree metadata across all base workspaces
	for _, ws := range m.workspaces {
...
// CloneWorkspace clones any remote Git repository URL and registers it as a workspace.
func (m *Manager) CloneWorkspace(repoURL, branch, targetName string) (*Workspace, error) {
	if strings.TrimSpace(repoURL) == "" {
		return nil, fmt.Errorf("repoURL cannot be empty")
	}
	...
	if name == "" {
		name = fmt.Sprintf("ws_%d", time.Now().Unix())
	}

	if existing, err := m.GetWorkspace(name); err == nil {
		return existing, nil
	}

	defaultWs, _ := m.GetWorkspace("default")
	var targetDir string
	if defaultWs != nil {
		targetDir = filepath.Join(defaultWs.Root, name)
	} else {
		targetDir = filepath.Join(".", name)
	}

	gitPath := filepath.Join(targetDir, ".git")
	if fi, err := os.Stat(gitPath); err == nil && (fi.IsDir() || fi.Mode().IsRegular()) {
		return m.RegisterWorkspace(name, name, targetDir)
	}
...
type VerificationResult struct {
	ProjectType string `json:"projectType"` // "go", "node", "python", "rust", "unknown"
...
// VerifyProject detects the project type in workspaceID and executes its verification test suite.
func (m *Manager) VerifyProject(workspaceID string, timeout time.Duration) (*VerificationResult, error) {
	ws, err := m.GetWorkspace(workspaceID)
	if err != nil {
		return nil, err
	}
	...
	if _, err := os.Stat(filepath.Join(ws.Root, "go.mod")); err == nil {
	...
	} else if _, err := os.Stat(filepath.Join(ws.Root, "package.json")); err == nil {
		projectType = "node"
		cmdString = "npm test"
		if runtime.GOOS == "windows" {
			cmd = exec.CommandContext(ctx, "cmd", "/c", "npm test")
		} else {
			cmd = exec.CommandContext(ctx, "npm", "test")
		}
	} else if _, err := os.Stat(filepath.Join(ws.Root, "Cargo.toml")); err == nil {
		projectType = "rust"
		cmdString = "cargo test"
		cmd = exec.CommandContext(ctx, "cargo", "test")
	} else if _, err := os.Stat(filepath.Join(ws.Root, "pytest.ini")); err == nil {
		projectType = "python"
		cmdString = "pytest"
		...
		return &VerificationResult{
			...
			Passed:      true,
			Command:     "none",
			Output:      "No recognized test runner (go.mod, package.json, Cargo.toml, pytest.ini) found.",
