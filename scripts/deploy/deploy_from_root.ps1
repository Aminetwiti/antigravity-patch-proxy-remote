# Antigravity Safe Deploy - Execute depuis la racine du projet
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "  Antigravity Safe Deploy Script" -ForegroundColor Yellow
Write-Host "============================================" -ForegroundColor Cyan

# 1. Antigravity calisirken dosyalari hazirla (kapatmadan)
Write-Host ""
Write-Host "[1/7] Antigravity kapatilmadan dosyalar hazirlaniyor..." -ForegroundColor Yellow
Write-Host "   NOT: Degisikliklerin etkili olmasi icin Antigravity'i manuel yeniden baslatin." -ForegroundColor Yellow
Write-Host "   OK" -ForegroundColor Green

# 2. Yollari tanimla - Use current directory
...
if (-not (Test-Path $srcDist)) {
    Write-Host "   HATA: dist klasoru bulunamadi: $srcDist" -ForegroundColor Red
    Write-Host "   Lutfen once 'npm run build' calistirin!" -ForegroundColor Yellow
    exit 1
}
...
if (Test-Path $ExePath) {
    ...
    Write-Host "    - Modal 2-step workflow implementé" -ForegroundColor Gray
    Write-Host "    - Providers hardcodés supprimés" -ForegroundColor Gray
    Write-Host "    - Support multi-modèles avec fetch API" -ForegroundColor Gray
    Write-Host "============================================" -ForegroundColor Cyan
} else {
